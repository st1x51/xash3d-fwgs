...
void R_StudioBuildNormalTable( void )
{
	...
	if( e->curstate.rendercolor.r || e->curstate.rendercolor.g || e->curstate.rendercolor.b )
		_TriColor4ub( e->curstate.rendercolor.r, e->curstate.rendercolor.g, e->curstate.rendercolor.b, 255 );
	else _TriColor4ub( 255, 255, 255, 255 );
}
...
void R_StudioRenderShadow( int iSprite, float *p1, float *p2, float *p3, float *p4 )
{
	...
	if( _TriSpriteTexture( gEngfuncs.pfnGetModelByIndex( iSprite ), 0 ))
	{
		_TriRenderMode( kRenderTransAlpha );
...
void R_StudioRenderModel( void )
{
	...
	if( RI.currententity->curstate.renderfx == kRenderFxGlowShell )
	{
		...
		_TriSpriteTexture( R_GetChromeSprite(), 0 );
